...
# Build script for Windows x64 - supports both lite and full variants
...
mv "$OUTPUT_DIR/LinqPadCompiler.exe" "$OUTPUT_DIR/linqpadcompiler.exe"
...
if [ "$VARIANT" == "full" ]; then
    ...
    # Download SDK
    SDK_URL="https://builds.dotnet.microsoft.com/dotnet/Sdk/${DOTNET_VERSION}/dotnet-sdk-${DOTNET_VERSION}-win-x64.zip"
    SDK_DIR="dist/sdk-temp"
    mkdir -p "$SDK_DIR"
    
    curl -fsSL "$SDK_URL" -o "$SDK_DIR/dotnet-sdk.zip"
...
EOF
    ...
    mv "$OUTPUT_DIR/linqpadcompiler.exe" "$OUTPUT_DIR/linqpadcompiler-bin.exe"
    mv "$OUTPUT_DIR/linqpadcompiler-wrapper.bat" "$OUTPUT_DIR/linqpadcompiler.bat"
...
cd dist
zip -r "linqpadcompiler-$VARIANT-$PLATFORM.zip" "$VARIANT-$PLATFORM/"
cd ..

# Show results
TOTAL_SIZE=$(du -sh "$OUTPUT_DIR" | cut -f1)
ARCHIVE_SIZE=$(du -h "dist/linqpadcompiler-$VARIANT-$PLATFORM.zip" | cut -f1)

echo "✓ Build complete!"
echo "  Directory: $OUTPUT_DIR ($TOTAL_SIZE)"
echo "  Archive: dist/linqpadcompiler-$VARIANT-$PLATFORM.zip ($ARCHIVE_SIZE)"